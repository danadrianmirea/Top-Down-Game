...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (IsKeyDown(KEY_A)) direction.x -= 1.0;   //move left
        if (IsKeyDown(KEY_D)) direction.x += 1.0;   //move right
        if (IsKeyDown(KEY_W)) direction.y -= 1.0;   //move up
        if (IsKeyDown(KEY_S)) direction.y += 1.0;   // move down
        ...
        if (runningTime >= updateTime)
        {
            ++knightFrame;
            runningTime = 0.f;
            if (knightFrame > maxKnightFrames) knightFrame = 0;
        }
